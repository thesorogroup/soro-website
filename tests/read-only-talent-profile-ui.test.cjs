...
test('the portal renderer routes Sales through the isolated profile and keeps a single Profile tab', () => {
  ...
  assert.match(operations, /sales:new Set\(\['overview','tasks','clients','talent-profile','placements','reports','help'\]\)/);
  assert.match(operations, /sales_management:new Set\(\['overview','tasks','clients','talent-profile','placements','reports','help'\]\)/);
