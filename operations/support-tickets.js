...
(function(root,factory){const api=factory(root);if(typeof module==='object'&&module.exports)module.exports=api;if(root)root.SoroSupportTickets=api;}(typeof globalThis!=='undefined'?globalThis:this,function(root){
  ...
  let node=null,version=0,options={},mountedActor='',selectedTicket=null,workspace=null,detailVersion=0,bindings=null,filters={status:'',team:'',assignment:'',offset:0};
  ...
  async function loadTickets(captured){
    ...
    const previousFocus=root.document?.activeElement,focusFilter=previousFocus?.dataset?.ticketFilter,focusStatus=previousFocus?.dataset?.ticketStatus;
    try{
      ...
      const filtered=Boolean(filters.status||filters.team||filters.assignment),total=Number.isSafeInteger(data.total)?data.total:null;
  ...
  function unmount(){version++;detailVersion++;bindings?.abort();bindings=null;node=null;options={};mountedActor='';selectedTicket=null;workspace=null;filters={status:'',team:'',assignment:'',offset:0};}
  function mount(container,config={}){
    ...
    node.addEventListener('click',async event=>{
      ...
      const shortcut=event.target.closest?.('[data-ticket-status]');if(shortcut){filters.status=filters.status===shortcut.dataset.ticketStatus?'':shortcut.dataset.ticketStatus;filters.offset=0;loadTickets(captured);return;}
      if(event.target.closest?.('[data-reset-ticket-filters]')){filters={status:'',team:'',assignment:'',offset:0};loadTickets(captured);return;}
  ...
  return Object.freeze({uploadMarkup,ticketMarkup,detailMarkup,summaryMarkup,readImage,pendingRequest,mount,unmount,refreshNotifications,MAX_BYTES,STATUSES,TEAMS});
}));
