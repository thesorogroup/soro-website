...
(function () {
  ...
  document.getElementById('notifications-dialog').addEventListener('click', event => {
    ...
    if(action.dataset.notificationTask){event.currentTarget.close();window.soroTaskDetail?.navigate(action.dataset.notificationTask);return;}
  ...
  root.addEventListener('submit', async event => {
    if (event.target.id === 'screening-results-form') {
      ...
      let internetSpeed;
      try {
        internetSpeed = screeningPresentation.serializeInternetSpeed({
          download: optionalText('internet_download_speed'),
          upload: optionalText('internet_upload_speed')
      ...
      window.dispatchEvent(new CustomEvent('soro:talent-screening-updated', { detail: { applicantId: applicant.id } }));
