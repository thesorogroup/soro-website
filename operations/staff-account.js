...
(function(root){
  ...
  async function openAccount(){
    ...
    try{
      ...
      form.querySelector('.record-manager-footer')?.insertAdjacentHTML('afterbegin',`<button type="button" class="admin-record-button" data-activity-kind="employee" data-activity-id="${esc(record.userId)}">Activity History</button>`);
  ...
  async function openOwnership(kind,id,initialField='review',afterSave){
    if(!isAdmin())return;
    const captured=scope(),dialog=shell('Edit ownership');
    dialog.classList.add('staff-ownership-dialog');
    try{
      ...
      dialog.querySelector('[data-staff-body]').innerHTML=`<form class="staff-ownership-form"><div class="staff-ownership-intro"><h3>${esc(workspace.record.name)}</h3><p>Change the responsible team member. The profile’s review stage stays the same.</p></div><div class="record-manager-field"><label>Responsibility<select name="field">${fields.map(([key,label])=>`<option value="${key}">${label}</option>`).join('')}</select></label></div><div class="record-manager-field"><label>Assigned to<select name="owner"></select></label></div>${kind==='client'?`<p class="record-manager-note">This also transfers the connected candidate workflow (${workspace.linkedTalentCount} Talent). Scheduled interviews must be resolved through the interview workflow first so calendar attendees stay correct.</p>`:'<p class="record-manager-note">To change Sales ownership for a Talent in a Client process, edit the owner on that Client’s profile. This keeps both records together.</p>'}<p role="status" aria-live="polite"></p><footer class="record-manager-footer"><button type="button" class="admin-record-button" data-cancel>Cancel</button><button type="submit" class="admin-record-button admin-record-button--primary">Save assignment</button></footer></form>`;
