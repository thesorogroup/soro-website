...
<section class="panel client-home-panel" id="client-home-talent" tabindex="-1"><div class="panel-head"><h2>Your talent</h2></div>${data.talent.length?data.talent.map(row=>`<article class="client-home-talent"><span class="client-home-avatar" aria-hidden="true">${escapeHtml((row.preferredName||row.fullName).slice(0,1))}</span><div><h3>${escapeHtml(row.preferredName||row.fullName)}</h3><p>${escapeHtml(row.roleTitle)}</p><span class="client-home-status">${({active:'Active',onboarding:'Onboarding',placement_confirmed:'Placement confirmed'})[row.status]}</span>${row.startDate?`<small>Starts ${escapeHtml(formatDate(row.startDate))}</small>`:''}${row.scheduleSummary?`<small>${escapeHtml(row.scheduleSummary)}</small>`:''}${actionButton('talent',row.applicantId,'View profile')}${actionButton('work-log',row.placementId,'Work Log & screenshots')}</div></article>`).join(''):'<div class="client-home-empty"><strong>Your team starts here</strong><p>Once a placement is confirmed, you can find your talent’s profile here.</p></div>'}</section>
  ...
  function click(event) {
    ...
    if(action==='work-log'&&workspace.talent.some(row=>row.placementId===id))options.onNavigate?.(action,id);
  }
