...
nav.addEventListener('click',e=>{const b=e.target.closest('[data-view]');if(!b||!viewAllowedForAuthenticatedRole(b.dataset.view))return;current=b.dataset.view;selectedTalentId=null;selectedClientId=null;history.pushState({},'',`${location.pathname}#${current}`);setActive();render();document.querySelector('.sidebar').classList.remove('open')});window.addEventListener('popstate',()=>{
  ...
  if(/^#documents(?:\?|$)/.test(location.hash)){selectedTalentId=null;selectedClientId=null;current='documents'}
...
if(/^#documents(?:\?|$)/.test(location.hash)){current='documents'}
...
else if(location.hash.slice(1) in data||['help','feedback','my-profile','client-talent-profile','client-candidate-review','client-shortlists','talent-my-profile','talent-review','available-talent'].includes(location.hash.slice(1))){current=location.hash.slice(1)}
render();
