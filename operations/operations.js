...
function render(){
  ...
  const talentWorkdayAction=role==='va'&&actualAuthenticatedRole()==='virtual_assistant'
    ...
    : '';
  const standardHeadingActions=`<button class="button primary" id="add-task">${primaryAction}</button>${current==='overview'||current==='clients'?`<button class="button" id="new-record">+ ${newAction}</button>`:''}${importAction}<button class="button">Customize</button>`;
  const headingActions=clientPortal||role==='va'
    ? (talentWorkdayAction?`<div class="heading-actions">${talentWorkdayAction}</div>`:'')
