...
const authenticatedEmployeeViews=Object.freeze({
  ...
  virtual_assistant:new Set(['overview','tasks','documents','help'])
});
function currentAuthenticatedRole(){return String(window.soroCurrentAccess?.role||'').toLowerCase()}
...
function viewDataForAuthenticatedRole(view,viewData){
  return authenticatedClientRoles.has(currentAuthenticatedRole())
    ? (clientSafeViewData[view]||viewData)
    : dataAllowedForAuthenticatedRole(view,viewData);
}
function syncAuthorizedNavigation(access=window.soroCurrentAccess){
  ...
  if(notificationsButton)notificationsButton.hidden=clientPortal;
  const overviewNav=document.getElementById('overview-nav');
  if(overviewNav)overviewNav.textContent=clientPortal?'Dashboard':'Overview';
...
function render(){
  ...
  if(current==='my-profile'){
    if(!authenticatedClientRoles.has(currentAuthenticatedRole())||!window.SoroClientProfile?.canOpenProfile()){
  ...
  const baseData=current==='overview'?(role==='admin'?data.overview:roleDashboards[role]):data[current];
  ...
  const headingActions=clientPortal?'':`<div class="heading-actions"><button class="button primary" id="add-task">${primaryAction}</button>${current==='overview'||current==='clients'?`<button class="button" id="new-record">+ ${newAction}</button>`:''}${importAction}<button class="button">Customize</button></div>`;
...
function applyRole(nextRole){role=nextRole;current='overview';const c=roleConfig[role];document.getElementById('role-label').textContent=c.label;document.querySelector('.profile strong').textContent=c.person;document.body.className=c.className;setActive();render();toast(`${role==='sales'?'Sales Panel':role==='talent'?'Talent Panel':role==='client'?'Client Portal':role==='va'?'Talent Portal':'Admin Panel'} preview is active.`)}document.getElementById('role-switcher').addEventListener('click',event=>{if(event.currentTarget.dataset.accountAction==='my-profile'){goToMyProfile();return}if(event.currentTarget.dataset.accountAction==='workspace-preview'||currentAuthenticatedRole()==='admin')document.getElementById('role-dialog').showModal()});document.getElementById('role-dialog').addEventListener('close',e=>{if(roleConfig[e.target.returnValue])applyRole(e.target.returnValue)});document.getElementById('global-search').addEventListener('keydown',e=>{if(e.key!=='Enter')return;if(authenticatedClientRoles.has(currentAuthenticatedRole())){if(!viewAllowedForAuthenticatedRole(current)){current='overview';history.replaceState({},'',`${location.pathname}#overview`);setActive();render()}toast('Client Portal search is not connected yet.');e.target.blur();return}if(viewAllowedForAuthenticatedRole('vas')){talentSearch=e.target.value;current='vas'}else if(viewAllowedForAuthenticatedRole('clients'))current='clients';else current='overview';setActive();render();e.target.blur()});
async function loadLiveApplicants(){if(!window.soroSupabase||!viewAllowedForAuthenticatedRole('vas')){liveApplicants=[];return}const fields='id,organization_id,auth_user_id,full_name,preferred_name,birth_date,gender_identity,gender_identity_self_description,pronouns,pronouns_self_description,email,phone,location,country,address_line_1,timezone,timezone_other_detail,status,work_status,work_status_other_detail,availability_note,expected_hourly_rate,expected_hourly_rate_max,expected_hourly_rate_text,education_level,greatest_dream,referral_source,dedicated_workspace,has_laptop,has_noise_canceling_headset,has_reliable_internet,has_backup_internet,has_emergency_workspace,equipment_summary,internet_summary,english_proficiency,assessment_summary,english_test_result,personality_profile_score,computer_specs,internet_speed,loom_video_url,resume_url,application_received_at,submitted_at,verified_skills,self_reported_experience_areas,self_reported_skills,other_experience_specialty,relevant_experience_years,relevant_experience_summary,education_training_summary,skill_profile_updated_at,talent_review_owner_id,sales_owner_id,talent_support_owner_id';const {data:applicants,error}=await window.soroSupabase.from('applicants').select(fields).is('archived_at',null).order('application_received_at',{ascending:false});if(error){liveApplicants=[];return}liveApplicants=applicants||[];if(current==='vas'||current==='talent-profile')render()}
...
window.addEventListener('soro-auth-changed',event=>{syncAuthorizedNavigation(event.detail.access);if(event.detail.session&&viewAllowedForAuthenticatedRole('vas'))loadLiveApplicants();else liveApplicants=[];render()});const initialHash=location.hash.match(/^#talent\/([^/]+)$/);if(initialHash){current='talent-profile';selectedTalentId=initialHash[1]}else if(location.hash.slice(1) in data||['my-profile','client-talent-profile'].includes(location.hash.slice(1))){current=location.hash.slice(1)}render();
