...
const {MAX_VIDEO_BYTES,VIDEO_TYPES,verifyStoredVideo}=require('./lib/talent-video-upload');
const RULES={profile_photo:{max:5242880,types:['image/jpeg','image/png']},resume:{max:10485760,types:['application/pdf','application/vnd.openxmlformats-officedocument.wordprocessingml.document']},introduction_video:{max:MAX_VIDEO_BYTES,types:VIDEO_TYPES}};
function validate(body){
 if(!body||typeof body!=='object'||Array.isArray(body)||!['prepare','complete'].includes(body.action))throw fail(400,'Choose a headshot, résumé, or introduction video upload.');
 ...
 if(!uuid(body.requestId)||!Object.hasOwn(RULES,body.kind))throw fail(400,'Choose a headshot, résumé, or introduction video upload.');
 const rule=RULES[body.kind];
 if(!rule.types.includes(body.type)||!Number.isInteger(body.size)||body.size<1||body.size>rule.max||typeof body.name!=='string'||!body.name.trim()||body.name.length>180||/[\x00-\x1f\x7f/\\]/.test(body.name))throw fail(400,'Use JPG or PNG up to 5 MB for a headshot; PDF or DOCX up to 10 MB for a résumé; MP4, WebM, or MOV up to 95 MiB for a video.');
 const ext=body.name.split('.').pop().toLowerCase();
 if(({jpg:'image/jpeg',jpeg:'image/jpeg',png:'image/png',pdf:'application/pdf',docx:'application/vnd.openxmlformats-officedocument.wordprocessingml.document',mp4:'video/mp4',webm:'video/webm',mov:'video/quicktime'})[ext]!==body.type)throw fail(400,'The file extension does not match its format.');
 return body;
}
...
async function handler(event){
 ...
 try{
  ...
  let verified;
  if(f.kind==='introduction_video')verified=await verifyStoredVideo(res,f.size,f.type);
  else {const bytes=await files.readBounded(res,f.size);verified={size:bytes.length,sha256:files.verifyBytes(bytes,f.type)};}
  const done=await rpc(user,{action:'finalize',fileId:f.fileId,sha256:verified.sha256,type:f.type,size:verified.size});
