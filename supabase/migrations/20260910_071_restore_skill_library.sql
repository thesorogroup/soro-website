-- Repair installations missing the optional Admin-managed library introduced
-- in 013. Never seed sample skills or change applicants' recorded skills.
begin;
...
create policy "internal users can read skill library"
  on public.skill_library for select to authenticated
...
create policy "soro admins can manage skill library"
  on public.skill_library for all to authenticated
...
revoke all on table public.skill_library from public, anon;
