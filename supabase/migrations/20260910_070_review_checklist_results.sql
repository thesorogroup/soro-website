-- Show entered results independently from source-file completion. Scores alone
-- must never satisfy the existing Bench Ready document requirements.
begin;

create or replace function private.talent_screening_result_recorded(p_value text)
returns boolean
  ...
  select exists (
    select 1 from (
      select regexp_replace(
        case when value ~* '^[[:space:]]*https?://' then ''
          else regexp_replace(value, '^[^:]*:[[:space:]]*', '') end,
        '^[[:space:]]+|[[:space:]]+$', '', 'g') as content
      from regexp_split_to_table(coalesce(p_value, ''), E'[|;\n]+') as value
    ) as parts
    where content <> ''
      and content !~* '^(pending|not recorded|not entered|not provided|unknown|n/?a|none|null|[-—–])$'
  );
$$;
revoke all on function private.talent_screening_result_recorded(text) from public, anon, authenticated;
...
as $$
  with source as (
    ...
    select item.*, source.id, source.organization_id
